...
public:
  ...
  virtual const vector< <Position<Bond>>* >& GetListeners() const;
  ...
  virtual void AddTrade(const Trade<Bond>& trade)
  {
   ...
   if(PositionMP.find(productID)== positions.end())
    {
    ...
    {
      PositionMP.UpdatePosition(book , quant);
    }
...
private:
  ...
  std::vector<ServiceListener<Trade<Bond>* > >TradeListerners;

};


class BondPositionServiceListener : ServiceListener<Trade <Bond> >
{
  public:
   
    BondPositionServiceListener(BondPositionService *bondPositionService_)
    {
      positionService = bondPositionService_;
    }
